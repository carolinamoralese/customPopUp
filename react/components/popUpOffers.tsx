...
const PopUpOfferts = ({ skuList, img, textAlert, activeAlert, timeToShowOffer }: Props) => {
    console.log(timeToShowOffer);
    const productContextValue = useProduct();
    const handles = useCssHandles(CSS_HANDLES);
    const [showAlertOffer, setShowAlertOffer] = useState(false);
    const [collectionId, setCollectionId] = useState("");
    const referenceIdValue = productContextValue?.selectedItem?.referenceId?.[0]?.Value;

    useEffect(() => {
        if (skuList.length > 0 && referenceIdValue) {
            const matchingSku = skuList.find(sku => sku.skudId.toString() === referenceIdValue);
            if (matchingSku) {
    ...
    if (activeAlert && showAlertOffer) {
        return (
            <div className={`fixed button-2 right-2 flex flex-column z-3000 ${handles["container-modal"]}`}>
                <img src={img.imgUrl} alt="Oferta" width={img.width} height={img.height} />
                <p>{textAlert}</p>
